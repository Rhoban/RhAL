...
void printDevice(RhAL::Device* dev)
{
  ...
  {
    // std::cout<<"    --" << it.first <<": "<<it.second->readValue().value<<" ReadOnly: "<<it.second->isReadOnly<<
    // std::endl; std::cout<<std::right<<std::setw(25)<<it.first<<": "<<it.second->readValue().value<<" ";
...
{
  std::cout << "\nTesting read/write\n"
            << "Dev: id:" << dev->id() << " name:" << dev->name() << std::endl;
  ...
  for (const auto& it : dev->registersList().containerBool())
  {
        ...
        it.first.compare("useValuesNow") && it.first.compare("positionTrackerOn"))
    {
      // for(int i=0;i<10;i++)
      {
        ...
        std::cout << std::right << std::setw(25) << it.first << ": wrote " << (tmpval.value ? "(false)" : "(true)")
                  << " read " << (tmpval2.value ? "(true)" : "(false)") << " time ("
                  << getTimeDuration<TimeDurationMicro>(tmpval.timestamp, tmpval2.timestamp).count() << "us"
                  << (it.second->isSlowRegister ? " slow register) " : ") ");
    ...
    {
      {
        ...
        std::cout << std::right << std::setw(25) << it.first << ": wrote (" << tmpval.value + delta << ") read ("
                  << tmpval2.value << ") duration ("
                  << getTimeDuration<TimeDurationMicro>(tmpval.timestamp, tmpval2.timestamp).count() << "us"
                  << (it.second->isSlowRegister ? " slow register) " : ") ");
    ...
    if (!it.second->isReadOnly)
    {
      // for(int i=0;i<10;i++)
      {
        ...
        */
        ...
        if (fabs(tmpval2.value - reallywritten) < it.second->getStepValue() / 2.0 ||
            fabs(tmpval2.value - reallywritten) < 0.001)
          res = true;

        float lost = fabs(towrite - reallywritten);

        std::cout << std::right << std::setw(25) << it.first << ": wrote (" << towrite << ") read (" << tmpval2.value
                  << ") duration (" << getTimeDuration<TimeDurationMicro>(tmpval.timestamp, tmpval2.timestamp).count()
                  << "us" << (it.second->isSlowRegister ? " slow register) " : ") ");
        /*
        if(res)
        {
            if(lost<0.001)
...
void ReadWriteTest(const std::string dev = "/dev/ttyACM0", int bauds = 1000000)
{
  ...
  manager.setProtocolConfig(dev, bauds, "DynamixelV1");
