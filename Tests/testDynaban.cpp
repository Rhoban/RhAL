...
void testDynaban()
{
  ...
  r = 1.0 / 5000.0;
  float torques1[5] = { 169.9973127875532f * r, 1.2118904739507608f * r, -859.49525560910968f * r,
                        109.93882674890278f * r, 489.17556618589202f * r };
  float torques2[5] = { 0.0f, -532.49287882689202f * r, -29.078490997017791f * r, 1058.1470413492527f * r,
                        -459.36643296722087f * r };
  float torques3[5] = { -169.99731278755326f * r, -1.2118904739506096f * r, 859.49525560910888f * r,
                        -109.93882674889758f * r, -489.17556618590021f * r };
  float torques4[5] = { 0.0f, 532.49287882689293f * r, 29.07849099701108f * r, -1058.1470413492355f * r,
                        459.3664329672057f * r };
  float torques[4][5];
    ...
    for (const auto& it : manager.devContainer<RhAL::Dynaban64>())
    {
      ...
      if (dev->isReadyForNextTrajectory())
      {
        ...
        for (int i = 6 - 1; i >= 0; i--)
        {
          ...
          {
            std::cout << "De-Synchronisation detected "
                         "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
                      << std::endl;
