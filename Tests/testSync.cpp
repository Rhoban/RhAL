...
int main(int argc, char* argv[])
{
  ...
  */
  ...
  for (int j = 0; j < 1000; j++)
  {
    ...
    tpr1 = getTimePoint();
    std::vector<ResponseState> states = protocol->syncRead(ids, addr_pos, datas, size);
    tpr2 = getTimePoint();
    ...
    for (size_t i = 0; i < states.size(); i++)
    {
      if (states[i] != ResponseOK)
        nberrors++;
      // printf("Err: 0x%x data: 0x%x 0x%x %f",states[i],(uint8_t) *datas[i],(uint8_t)
      // *(datas[i]+1),convDecode_PositionRx(datas[i]));
      printf("id: %d RetCode: 0x%x data: 0x%x 0x%x\n", (int)ids[i], states[i], (uint8_t)*datas[i],
             (uint8_t) * (datas[i] + 1));
    }
    std::cout << std::endl;
    ...
    // usleep(100);
    tpr1 = getTimePoint();
    protocol->syncWrite((const std::vector<RhAL::id_t>)ids, addr_goal, dataswrite, size);
