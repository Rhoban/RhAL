...
namespace RhAL
{
Register::Register(const std::string& name, addr_t addr, size_t length, unsigned int periodPackedRead, bool isForceRead,
                   bool isForceWrite, bool isSlowRegister, bool isReadOnly)
  :  // Member init
  id(0)
  , name(name)
  , addr(addr)
  ...
  , _mutex()
{
  ...
  {
    throw std::logic_error("Register length invalid with static max length: " + name);
  }
  if (addr + length >= AddrDevLen)
  {
    throw std::logic_error("Register address/length is outside static memory range: " + name);
  }
}

void Register::init(id_t tmpId, CallManager* manager, data_t* bufferRead, data_t* bufferWrite)
{
  if (manager == nullptr)
  {
    throw std::logic_error("Register null manager pointer:" + name);
  }
  if (bufferRead == nullptr || bufferWrite == nullptr)
  {
    throw std::logic_error("Register null buffer pointer:" + name);
  }
  id = tmpId;
  _manager = manager;
...
template <typename T>
TypedRegister<T>::TypedRegister(const std::string& name, addr_t addr, size_t length, FuncConvEncode<T> funcConvEncode,
                                FuncConvDecode<T> funcConvDecode, unsigned int periodPackedRead, bool forceRead,
                                bool forceWrite, bool isSlowRegister)
  :  // Member init
  Register(name, addr, length, periodPackedRead, forceRead, forceWrite, isSlowRegister)
...
template <typename T>
TypedRegister<T>::TypedRegister(const std::string& name, addr_t addr, size_t length, FuncConvDecode<T> funcConvDecode,
                                unsigned int periodPackedRead, bool forceRead, bool forceWrite, bool isSlowRegister)
  :  // Member init
  Register(name, addr, length, periodPackedRead, forceRead, forceWrite, isSlowRegister, true)
  , funcConvEncode()
  , funcConvDecode(funcConvDecode)
  , _minValue(T(0))
  , _maxValue(T(0))
...
void TypedRegister<T>::writeValue(T val, bool noCallback)
{
  ...
  if (isReadOnly)
  {
    throw std::logic_error("TypedRegister write to read only Register: " + name);
  }
  ...
  {
    _valueWrite = aggregateValue(_aggregationPolicy, _valueWrite, val);
  }
  else
  {
    _valueWrite = val;
...
T TypedRegister<T>::getWrittenValueAfterEncode() const
{
  ...
  if (isReadOnly)
  {
    throw std::logic_error("TypedRegister get write value on read only Register: " + name);
  }
...
void TypedRegister<T>::doConvEncode()
{
  ...
  if (isReadOnly)
  {
    throw std::logic_error("TypedRegister conv encode on read only Register: " + name);
  }
