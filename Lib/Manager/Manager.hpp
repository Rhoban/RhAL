...
public:
   ...
   * Initialization
   */
  inline Manager() : AggregateManager<Types...>(), _managerThread(nullptr), _managerThreadContinue(false)
  {
  }
  ...
  {
    std::lock_guard<std::mutex> lock(CallManager::_mutex);
    if (!j.isObject() || j.size() > sizeof...(Types) + 1 || j["Manager"].isNull() || j["Protocol"].isNull())
    {
      throw std::runtime_error("Manager load parameters root json malformed");
    }
   ...
   * in JSON format
   */
  inline virtual void writeConfig(const std::string& filename) const override
  {
    std::ofstream file;
    file.open(filename);
    if (!file.is_open())
    {
      throw std::runtime_error("Manager unable to write file: " + filename);
    }
   ...
   * file is malformated
   */
  inline virtual void readConfig(const std::string& filename) override
  {
    std::ifstream file(filename, std::ios::in | std::ios::binary);
    if (!file.is_open())
    {
      throw std::runtime_error("Manager unable to read file: " + filename);
    }
    std::string contents;
    ...
    {
      throw std::runtime_error("Rhal::Manager:readConfig: Error while reading file '" + filename +
  ...
  inline void startManagerThread(std::function<void()> callback = []() {})
  {
    if (_managerThread == nullptr)
    {
      ...
      _managerThread = new std::thread([this, callback]() {
