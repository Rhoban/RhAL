...
class AggregateManager : public BaseManager, public ImplManager<Types>...
{
  ...
  // is not empty
  static_assert(sizeof...(Types) != 0, "AggregateManager empty variatic template types");
...
public:
  ...
  using TypeInPack = typename std::enable_if<is_type_in_pack<T, Types...>::value, U>;
  template <typename T, typename U>
  using TypeNotInPack = typename std::enable_if<!is_type_in_pack<T, Types...>::value, U>;
  ...
  inline void devAdd(id_t id, const std::string& name)
  {
    ...
    {
      throw std::logic_error("AggregateManager device name or id already added: " + name);
    }
    else
    {
   ...
   */
  inline virtual void devAddByTypeNumber(id_t id, type_t type) override
  {
  ...
  inline typename TypeNotInPack<T, const T&>::type devById(id_t id) const
  {
    ...
    {
      throw std::logic_error("AggregateManager Device id not found: " + std::to_string(id));
    }
    try
    {
    ...
    {
      throw std::logic_error("AggregateManager bad Device cast: " + std::to_string(id));
    }
  }
  // Return T&
  template <typename T>
  ...
  // Return const T&
  template <typename T>
  inline typename TypeInPack<T, const T&>::type devByName(const std::string& name) const
  {
    return ImplManager<T>::devByName(name);
  }
  // Return T&
  template <typename T>
  inline typename TypeInPack<T, T&>::type devByName(const std::string& name)
  {
    return ImplManager<T>::devByName(name);
  }
  // Return const T&
  template <typename T>
  inline typename TypeNotInPack<T, const T&>::type devByName(const std::string& name) const
  {
    if (_devicesByName.count(name) == 0)
    {
      throw std::logic_error("AggregateManager Device name not found: " + name);
    }
    try
    {
    ...
    {
      throw std::logic_error("AggregateManager bad Device cast: " + name);
    }
  }
  // Return T&
  template <typename T>
  inline typename TypeNotInPack<T, T&>::type devByName(const std::string& name)
  {
    if (_devicesByName.count(name) == 0)
    {
      throw std::logic_error("AggregateManager Device name not found: " + name);
...
protected:
   ...
   * into a json object
   */
  inline Json::Value saveAggregatedJSON() const
  {
    Json::Value j;
   ...
   */
  inline void loadAggregatedJSON(const Json::Value& j)
  {
...
private:
  ...
  {
    // Add by type number
    inline static void runAddByType(AggregateManager<Types...>* ptr, id_t id, type_t type)
    {
      if (ImplManager<T>::typeNumber() == type)
      {
        std::string name =
            ImplManager<T>::typeName() + "_" + std::to_string(ptr->ImplManager<T>::devContainer().size() + 1);
        ptr->devAdd<T>(id, name);
      }
      else
      {
        throw std::logic_error("AggregateManager add Device of type not supported: " + std::to_string(id));
      }
    }
    // Is type supported
    ...
    // Type number by Id
    inline static type_t runTypeNumberById(const AggregateManager<Types...>* ptr, id_t id)
    {
      if (!ptr->devExistsById<T>(id))
      {
        throw std::logic_error("AggregateManager type Device id not found: " + std::to_string(id));
      }
      return ImplManager<T>::typeNumber();
    }
    // Type name by Id
    inline static std::string runTypeNameById(const AggregateManager<Types...>* ptr, id_t id)
    {
      if (!ptr->devExistsById<T>(id))
      {
        throw std::logic_error("AggregateManager type Device id not found: " + std::to_string(id));
    ...
    // Type number by Name
    inline static type_t runTypeNumberByName(const AggregateManager<Types...>* ptr, const std::string& name)
    {
      if (!ptr->devExistsByName<T>(name))
      {
        throw std::logic_error("AggregateManager type Device name not found: " + name);
      }
      return ImplManager<T>::typeNumber();
    }
    // Type name by Name
    inline static std::string runTypeNameByName(const AggregateManager<Types...>* ptr, const std::string& name)
    {
      if (!ptr->devExistsByName<T>(name))
      {
        throw std::logic_error("AggregateManager type Device name not found: " + name);
    ...
    // Save JSON
    inline static void runSaveJSON(const AggregateManager<Types...>* ptr, Json::Value& j)
    {
      Json::Value tmpJ = ptr->ImplManager<T>::saveJSON();
    ...
    // Load JSON
    inline static void runLoadJSON(AggregateManager<Types...>* ptr, const Json::Value& j)
    {
      if (!j[ImplManager<T>::typeName()].isNull())
      {
        // Check json format and
        // add non existing devices
        ptr->checkDevicesJSON<T>(j[ImplManager<T>::typeName()]);
        // Load parameters
        ptr->ImplManager<T>::loadJSON(j[ImplManager<T>::typeName()]);
      }
    }
  };
   ...
   */
  template <typename T>
  inline void checkDevicesJSON(const Json::Value& j)
  {
    // Check json type
    if (!j.isObject())
    {
      throw std::runtime_error("AggregateManager load parameters json not object");
    }
    // Load TypedManager parameters
    if (!j["parameters"].isNull() && !j["parameters"].isObject())
    {
      throw std::runtime_error("AggregateManager load parameters: parameters is not null and not an object");
    }
    // Check device exists and type
    const Json::Value& devices = j["devices"];
    if (devices.isNull())
    {
      throw std::runtime_error("AggregateManager load parameters no devices key");
    }
    if (!devices.isArray())
    {
      throw std::runtime_error("AggregateManager load parameters devices not array");
    }
    // Check no other key exists
    if (j.size() != 2)
    {
      throw std::runtime_error("AggregateManager load parameters json malformed");
    }
    ...
    for (Json::Value::ArrayIndex i = 0; i < devices.size(); i++)
    {
      ...
      if (dev.isNull())
      {
        throw std::out_of_range("AggregateManager load parameters device not found");
      }
      // Check json formated
      if (!dev.isObject())
      {
        throw std::runtime_error("AggregateManager load parameters device not object");
      }
      if (dev["id"].isNull() || dev["name"].isNull() ||
          (!dev["parameters"].isNull() && !dev["parameters"].isObject()) || !dev["id"].isInt() ||
          !dev["name"].isString() || dev.size() != 3)
      {
        throw std::runtime_error("AggregateManager load parameters device json malformated");
      }
      // Retrieve device id and name
      id_t id = dev["id"].asInt();
      const std::string& name = dev["name"].asString();
      // Check device exist
      ...
      if (!isById && !isByName)
      {
        ...
        devAdd<T>(id, name);
          ...
          (isById && !isByName) || (!isById && isByName) || !isByIdTyped || !isByNameTyped)
      {
        throw std::runtime_error("AggregateManager load parameters device id/name problem: " + name);
      }
    }
  }
};
