...
namespace RhAL
{
ParametersList::ParametersList() : _paramsBool(), _paramsNumber(), _paramsStr()
{
}

bool ParametersList::exists(const std::string& name) const
{
  return (_paramsBool.count(name) != 0) || (_paramsNumber.count(name) != 0) || (_paramsStr.count(name) != 0);
}
...
void ParametersList::add(ParameterBool* param)
{
  ...
  {
    throw std::logic_error("ParametersContainer bool name already exists: " + param->name);
  }
...
void ParametersList::add(ParameterNumber* param)
{
  ...
  if (exists(param->name))
  {
    throw std::logic_error("ParametersContainer number name already exists: " + param->name);
  }
...
void ParametersList::add(ParameterStr* param)
{
  ...
  if (exists(param->name))
  {
    throw std::logic_error("ParametersContainer string name already exists: " + param->name);
  }
...
const ParameterBool& ParametersList::paramBool(const std::string& name) const
{
  ...
  {
    throw std::logic_error("ParametersContainer bool name does not exist: " + name);
  }
...
const ParameterNumber& ParametersList::paramNumber(const std::string& name) const
{
  ...
  {
    throw std::logic_error("ParametersContainer number name does not exist: " + name);
  }
...
const ParameterStr& ParametersList::paramStr(const std::string& name) const
{
  ...
  {
    throw std::logic_error("ParametersContainer str name does not exist: " + name);
  }
...
void ParametersList::loadJSON(const Json::Value& j)
{
  ...
  if (!j.isObject())
  {
    throw std::runtime_error("ParametersContainer load parameters json not object");
  }
  ...
  for (Json::Value::const_iterator it = j.begin(); it != j.end(); it++)
  {
    ...
    if (v.isBool())
    {
      ...
      {
        throw std::runtime_error("ParametersContainer load parameters json bool does not exist: " + key);
      }
      else
      {
    ...
    else if (v.isDouble())
    {
      ...
      {
        throw std::runtime_error("ParametersContainer load parameters json number does not exist: " + key);
      }
      else
      {
    ...
    else if (v.isString())
    {
      ...
      {
        throw std::runtime_error("ParametersContainer load parameters json str does not exist: " + key +
                                 " with value: " + v.asString());
      }
      else
      {
    ...
    {
      throw std::runtime_error("ParametersContainer load parameters json malformated");
    }
  }
}
