...
const Register& RegistersList::reg(const std::string& name) const
{
  if (!exists(name))
  {
    throw std::logic_error("RegistersList name does not exists: " + name);
  }
...
void RegistersList::checkMemorySpace(const Register* reg) const
{
  if (reg == nullptr)
  {
    throw std::logic_error("RegistersList null pointer");
  }
  if (exists(reg->name))
  {
    throw std::logic_error("RegistersList name already added: " + reg->name);
  }
  if (_manager == nullptr)
  {
    throw std::logic_error("RegistersList manager pointer not initialized");
  }
  for (const auto& it : _registers)
  {
    const Register* reg2 = it.second;
    if ((reg->addr > reg2->addr && reg->addr < reg2->addr + reg2->length) ||
        (reg->addr + reg->length > reg2->addr && reg->addr + reg->length < reg2->addr + reg2->length))
    {
      throw std::logic_error("RegistersList register memory intersection: " + reg->name + " and " + reg2->name);
    }
  }
}
...
void RegistersList::initRegister(Register* reg)
{
  ...
  reg->init(_id, _manager, _memorySpaceRead + reg->addr, _memorySpaceWrite + reg->addr);
