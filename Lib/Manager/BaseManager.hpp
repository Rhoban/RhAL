...
public:
   ...
   */
  template <typename T>
  using DevicesByName = std::map<std::string, T*>;
  template <typename T>
  using DevicesById = std::map<id_t, T*>;
   ...
   */
  virtual void onNewRegister(id_t id, const std::string& name) override;
   ...
   */
  virtual void forceRegisterRead(id_t id, const std::string& name) override;
  virtual void forceRegisterWrite(id_t id, const std::string& name) override;
   ...
   * and protocol name.
   */
  void setProtocolConfig(const std::string& port, unsigned long baudrate, const std::string& protocol);
...
private:
   ...
   * write are selected.
   */
  std::vector<BatchedRegisters> computeBatchedRegisters(bool isReadOrWrite);
   ...
   * not be used.
   */
  bool checkResponseState(ResponseState state, Device* dev);
