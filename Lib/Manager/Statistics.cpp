...
namespace RhAL
{
Statistics::Statistics() : managerStart(getTimePoint())
{
  reset();
}
...
void Statistics::print(std::ostream& os) const
{
  ...
  os << "------------------------" << std::endl;
  os << "Time since Manager creation: " << duration_float(managerStart, now) << "s" << std::endl;
  ...
  if (flushCount > 1)
  {
    os << "Mean Manager flush() period: " << duration_float(sumFlushPeriod) / (flushCount - 1.0) << "s" << std::endl;
  }
  os << "Max Manager flush() period: " << duration_float(maxFlushPeriod) << "s" << std::endl;
  ...
  os << "Waiting in flush() spent time: " << duration_float(waitUsersDuration) << "s" << std::endl;
  os << "Waiting in WaitNextFlush() spent time: " << duration_float(waitManagerDuration) << "s" << std::endl;
  ...
  os << "Read() sum spent time: " << duration_float(sumReadDuration) << "s" << std::endl;
  if (readCount > 0)
  {
    os << "Read() mean spent time: " << duration_float(sumReadDuration) / readCount << "s" << std::endl;
  }
  os << "Read() max spent time: " << duration_float(maxReadDuration) << "s" << std::endl;
  os << "Write() calls: " << writeCount << std::endl;
  os << "Write() bytes length: " << writeLength << std::endl;
  os << "Write() sum spent time: " << duration_float(sumWriteDuration) << "s" << std::endl;
  if (writeCount > 0)
  {
    os << "Write() mean spent time: " << duration_float(sumWriteDuration) / writeCount << "s" << std::endl;
  }
  os << "Write() max spent time: " << duration_float(maxWriteDuration) << "s" << std::endl;
  os << "SyncRead() calls: " << syncReadCount << std::endl;
  os << "SyncRead() bytes length: " << syncReadLength << std::endl;
  os << "SyncRead() sum spent time: " << duration_float(sumSyncReadDuration) << "s" << std::endl;
  if (syncReadCount > 0)
  {
    os << "SyncRead() mean spent time: " << duration_float(sumSyncReadDuration) / syncReadCount << "s" << std::endl;
  }
  os << "SyncRead() max spent time: " << duration_float(maxSyncReadDuration) << "s" << std::endl;
  os << "SyncWrite() calls: " << syncWriteCount << std::endl;
  os << "SyncWrite() bytes length: " << syncWriteLength << std::endl;
  os << "SyncWrite() sum spent time: " << duration_float(sumSyncWriteDuration) << "s" << std::endl;
  if (syncWriteCount > 0)
  {
    os << "SyncWrite() mean spent time: " << duration_float(sumSyncWriteDuration) / syncWriteCount << "s" << std::endl;
  }
  os << "SyncWrite() max spent time: " << duration_float(maxSyncWriteDuration) << "s" << std::endl;
