...
public:
   ...
   */
  void prepareFirstTrajectory(float* positionCoefs, int nbPositionCoefs, float* torqueCoefs, int nbTorqueCoefs,
                              float duration);

  /*As of 25/02/2016 :
   *1 : Predictive command only. Follows the trajectory set in the traj1 fields but only relying on the model of the
   *motor. This mode can be useful when calibrating the model 2 : PID only. Follows the trajectory set in the traj1
   *fields but only relying on the PID. 3 : PID and predictive command. Follows the trajectory set in the traj1 fields
   *using both the PID and the predictive command. This should be the default mode when following a trajectory
   */
  ...
  /*
   * This function should be called only if copyNextBuffer is false, which means that Dynaban is ready to receive a
   * trajectory (that will be bufferized until the current trajectory is finished). The only situation where it'd make
   * sense to call this function if copyNextBuffer is true is if we've sent a continuation trajectory that hasn't yet
   * been used and we want to change it before Dynaban finishes its current traj. Ofc, not recommended.
   */
  void updateNextTrajectory(float* positionCoefs, int nbPositionCoefs, float* torqueCoefs, int nbTorqueCoefs,
                            float duration);

  /**
   * Tells the servo to stop once the current trajectory is over. The calls to updateNextTrajectory set the
   * copyNextBuffer flag to 1, if stopAtTheEndOfTheTrajectory is not called, the servo will swap the buffers one last
   * time, replaying the penultimate trajectory [disastrous].
   */
   ...
   *0 : Default mode. Uses the PID to follow the goal position. The behaviour should be almost identical to the default
   *firmware 1 : Predictive command only. Follows the trajectory set in the traj1 fields but only relying on the model
   *of the motor. This mode can be useful when calibrating the model 2 : PID only. Follows the trajectory set in the
   *traj1 fields but only relying on the PID. 3 : PID and predictive command. Follows the trajectory set in the traj1
   *fields using both the PID and the predictive command. This should be the default mode when following a trajectory 4
   *: Compliant-kind-of mode. In this mode, the servo will try to act compliant
   */
  ...
  /**
   * PWM control voltage in V
   */
  TypedRegisterFloat& voltagePWM();
...
protected:
  ...
  TypedRegisterFloat _voltagePWM;             // 1 DA
};
