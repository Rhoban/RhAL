...
class RX : public DXL
{
    public:
        ...
        inline RX(const std::string& name, id_t id) :
			...
			_position("position", 0x24, 2, convDecode_PositionRx, 1, false),
			_speed("speed", 0x26, 2,  convDecode_SpeedRx, 0),
			_load("load", 0x28, 2,  convDecode_torque, 0),
			_voltage("voltage", 0x2A, 1,  convDecode_voltage, 20),
			_temperature("temperature", 0x2B, 1,  convDecode_temperature, 20),
