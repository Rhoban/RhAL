...
// All the positions have an offset of 180 degrees (also true for the MXs and the RXs). This is because dxl's zeros are
// not the same among different servomotors, so we unified them.
#define OFFSET 180
...
Dynaban64::Dynaban64(const std::string& name, id_t id)
  : MX64(name, id)
  ,
  //_register("name", address, size, encodeFunction, decodeFunction, updateFreq, forceRead=true, forceWrite=false,
  // isSlow=false)
  ...
  , _outputTorque("ouputTorque", 0xC6, 4, convDecode_float, 0, false)
  ,  // Read only
  _electricalTorque("electricalTorque", 0xCA, 4, convDecode_float, 0)
  ,  // Read Only
...
void Dynaban64::prepareFirstTrajectory(float* positionCoefs, int nbPositionCoefs, float* torqueCoefs, int nbTorqueCoefs,
                                       float duration)
{
  ...
  if (nbPositionCoefs != 0)
  {
    ...
    setPositionTrajectory1(fivePositionCoefs[0], fivePositionCoefs[1], fivePositionCoefs[2], fivePositionCoefs[3],
                           fivePositionCoefs[4]);
  ...
  if (nbTorqueCoefs != 0)
  {
    ...
    // fivePositionCoefs contains always 5 coefs, the lasts ones are 0.0 if the user asked for less than 5 coefs.
    setTorqueTrajectory1(fivePositionCoefs[0], fivePositionCoefs[1], fivePositionCoefs[2], fivePositionCoefs[3],
                         fivePositionCoefs[4]);
    torquePoly1Size() = nbTorqueCoefs;
  }
  /*
   * We set copy next buffer to 0, this means that if nothing is done the movement will stop at the end of the current
   *traj. But it also means that the firmware is ready to receive a second trajectory that, if transmitted, will be used
   *once the first one has ended.
   */
  copyNextBuffer() = 0;
}
...
void Dynaban64::updateNextTrajectory(float* positionCoefs, int nbPositionCoefs, float* torqueCoefs, int nbTorqueCoefs,
                                     float duration)
{
  float fivePositionCoefs[5];
  ...
  if (nbPositionCoefs != 0)
  {
    ...
    // fivePositionCoefs contains always 5 coefs, the lasts ones are 0.0 if the user asked for less than 5 coefs.
    setPositionTrajectory2(fivePositionCoefs[0], fivePositionCoefs[1], fivePositionCoefs[2], fivePositionCoefs[3],
                           fivePositionCoefs[4]);
  ...
  if (nbTorqueCoefs != 0)
  {
    ...
    // fivePositionCoefs contains always 5 coefs, the lasts ones are 0.0 if the user asked for less than 5 coefs.
    setTorqueTrajectory2(fivePositionCoefs[0], fivePositionCoefs[1], fivePositionCoefs[2], fivePositionCoefs[3],
                         fivePositionCoefs[4]);
