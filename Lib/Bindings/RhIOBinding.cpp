...
namespace RhAL
{
RhIOBinding::RhIOBinding(BaseManager& manager, const std::string& nodeName, bool isUpdateThread)
  : _thread(nullptr), _isOver(true), _manager(&manager), _node(nullptr)
{
  ...
  // Create RhIO command
  _node->newCommand("rhalScan", "Force the bus scan", std::bind(&RhIOBinding::cmdScan, this, std::placeholders::_1));
  ...
  for (const auto& device : allDevices)
  {
    ...
    for (const auto& reg : device.second->registersList().containerBool())
    {
      ...
      if (registersNode->getValueType(reg.first) == RhIO::NoValue)
      {
        ...
        registersNode->setCallbackBool(reg.first, [reg](bool newValue) { reg.second->writeValue(newValue, true); });
        auto callback = [registersNode, reg](bool newValue) { registersNode->setBool(reg.first, newValue, true); };
    ...
    for (const auto& reg : device.second->registersList().containerInt())
    {
      ...
      if (registersNode->getValueType(reg.first) == RhIO::NoValue)
      {
        ...
        registersNode->setCallbackInt(reg.first, [reg](long newValue) { reg.second->writeValue(newValue, true); });
        auto callback = [registersNode, reg](long newValue) { registersNode->setInt(reg.first, newValue, true); };
        reg.second->setCallbackRead(callback);
        reg.second->setCallbackWrite(callback);
      }
    }
    ...
    for (const auto& reg : device.second->registersList().containerFloat())
    {
      ...
      if (registersNode->getValueType(reg.first) == RhIO::NoValue)
      {
        ...
        registersNode->setCallbackFloat(reg.first, [reg](double newValue) { reg.second->writeValue(newValue, true); });
        auto callback = [registersNode, reg](double newValue) { registersNode->setFloat(reg.first, newValue, true); };
        reg.second->setCallbackRead(callback);
        reg.second->setCallbackWrite(callback);
      }
    }
...
void RhIOBinding::updateParameters(ParametersList& params, RhIO::IONode* node)
{
  ...
  for (const auto& param : params.containerBool())
  {
    if (node->getValueType(param.first) == RhIO::NoValue)
    {
      ...
      node->setCallbackBool(param.first, [param](bool newValue) { param.second->value = newValue; });
    }
    else
    {
      node->setBool(param.first, param.second->value);
  ...
  for (const auto& param : params.containerNumber())
  {
    if (node->getValueType(param.first) == RhIO::NoValue)
    {
      ...
      node->setCallbackFloat(param.first, [param](double newValue) { param.second->value = newValue; });
    }
    else
    {
      node->setFloat(param.first, param.second->value);
  ...
  for (const auto& param : params.containerStr())
  {
    if (node->getValueType(param.first) == RhIO::NoValue)
    {
      ...
      node->setCallbackStr(param.first, [param](std::string newValue) { param.second->value = newValue; });
    }
    else
    {
      node->setStr(param.first, param.second->value);
...
std::string RhIOBinding::cmdScan(std::vector<std::string> argv)
{
  (void)argv;
  _manager->scan();
  update();
...
std::string RhIOBinding::cmdStats(std::vector<std::string> argv)
{
  (void)argv;
...
std::string RhIOBinding::cmdResetStats(std::vector<std::string> argv)
{
  (void)argv;
  std::ostringstream oss;
...
std::string RhIOBinding::cmdReadDev(std::vector<std::string> argv)
{
  if (argv.size() != 1)
  {
...
std::string RhIOBinding::cmdReadReg(std::vector<std::string> argv)
{
  if (argv.size() != 2)
  {
...
std::string RhIOBinding::cmdPing(std::vector<std::string> argv)
{
  if (argv.size() != 1)
  {
...
std::string RhIOBinding::cmdStatus(std::vector<std::string> argv)
{
  (void)argv;
  const auto& allDevices = _manager->devContainer();
  ...
  for (const auto& dev : allDevices)
  {
    ...
    {
      if (dev.second->lastFlags() & ResponseOverload)
        os << " Overload";
      if (dev.second->lastFlags() & ResponseOverheat)
        os << " Overheat";
      if (dev.second->lastFlags() & ResponseBadVoltage)
        os << " BadVoltage";
      if (dev.second->lastFlags() & ResponseQuiet)
        os << " Quiet";
      if (dev.second->lastFlags() & ResponseBadChecksum)
        os << " BadChecksum";
      if (dev.second->lastFlags() & ResponseDeviceBadInstruction)
        os << " DeviceBadInstruction";
      if (dev.second->lastFlags() & ResponseDeviceBadChecksum)
        os << " DeviceBadChecksum";
      if (dev.second->lastFlags() & ResponseBadSize)
        os << " BadSize";
      if (dev.second->lastFlags() & ResponseBadProtocol)
        os << " BadProtocol";
      if (dev.second->lastFlags() & ResponseBadId)
        os << " BadId";
    }
    else
    {
...
std::string RhIOBinding::cmdCheck(std::vector<std::string> argv)
{
  (void)argv;
...
std::string RhIOBinding::cmdSaveConf(std::vector<std::string> argv)
{
  if (argv.size() != 1)
  {
    return "Invalid Argument. Usage:\nsaveConf [filepath]";
...
std::string RhIOBinding::cmdLoadConf(std::vector<std::string> argv)
{
  if (argv.size() != 1)
  {
    return "Invalid Argument. Usage:\nloadConf [filepath]";
...
std::string RhIOBinding::cmdEmergency(std::vector<std::string> argv)
{
  (void)argv;
...
std::string RhIOBinding::cmdEmergencyExit(std::vector<std::string> argv)
{
  (void)argv;
...
std::string RhIOBinding::cmdInit(std::vector<std::string> argv)
{
  (void)argv;
  const auto& allDevices = _manager->devContainer();
...
std::string RhIOBinding::cmdChangeId(std::vector<std::string> argv)
{
  ...
  return "Change id from" + argv[0] + " to " + argv[1] + ". RhAL Exit.";
}

std::string RhIOBinding::cmdTare(std::vector<std::string> argv)
{
  (void)argv;
...
std::string RhIOBinding::cmdGyroTare(std::vector<std::string> argv)
{
  (void)argv;
