...
typedef RhAL::Manager<RhAL::ExampleDevice1, RhAL::RX64, RhAL::ExampleDevice2> Manager;
...
int main()
{
  ...
  manager.setProtocolConfig("", 1000000, "FakeProtocol");
  ...
  for (const auto& it : manager.devContainer())
  {
    ...
    (void)typebis;
    std::cout << "Device: name:" << it.first << " id:" << it.second->id() << " typeName:" << name
              << " typeNumber:" << type << std::endl;
  }
  ...
  std::map<std::string, RhAL::BaseExampleDevice1*> container = manager.devContainer<RhAL::BaseExampleDevice1>();
